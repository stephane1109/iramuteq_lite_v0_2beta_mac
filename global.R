...
installer_environnement_application <- function(envname = NULL) {
  ...
  ok_python <- tryCatch(
    installer_packages_python_lda(),
    error = function(e) {
      message("installer_environnement_application/install_packages_python_lda error: ", conditionMessage(e))
      FALSE
    }
  )

  invisible(list(ok_r = ok_r, ok_python = ok_python, envname = envname))
}
